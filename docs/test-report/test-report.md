...
- 인증, 사용자/프로필, 학습 일정/칸반 태스크, 학습 노트, 집중 시간/통계, 보상, 포인트 상점, 커뮤니티 게시글/댓글/반응/북마크/내 북마크 목록/사용자 신고, 관리자 커뮤니티 신고 처리, 관리자, AI 학습 지원 API 테스트 결과 기록
...
| 학습 노트 API | 계획됨 | 완료 | PR #81, 학습 노트 API 테스트 완료 | 학습 노트 프론트 화면 연결 |
...
| 보상/퀘스트/뱃지/포인트 | 계획됨 | 부분 구현 | Reward API, 보상 schema/migration, 보상 API 테스트, 포인트 상점 MVP 테스트 반영 | 보상 화면, 관리자용 보상 관리, 보상 정책 고도화 |
...
| TC-INT-012 | API 통합 테스트 | 커뮤니티 반응 API | 게시글 반응 LIKE/DISLIKE 생성, 동일 type 재요청, 반응 전환, 반응 취소, invalid postId, type validation, userId spoofing 차단, 다른 사용자 반응 영향 없음 검증 | `npm test` | 400/401/201/200/404 응답 및 현재 사용자 기준 반응 변경 제한 | 통과 |
...
| TC-INT-017 | API 통합 테스트 | 포인트 상점 API | 상점 목록, 내 상점 상태, 구매, 중복 구매 차단, 포인트 부족, 구매 전 적용 차단, 적용/해제, 민감정보 미노출 검증 | `npm test`, `npm --prefix src/backend test -- --runTestsByPath tests/shop.test.js` | 400/401/409/200/201 응답 및 현재 사용자 기준 포인트 상점 처리 | 통과 |
| TC-INT-018 | API 통합 테스트 | 서비스 점검/관리자 공지 API | 공개 점검 상태 조회, 관리자 점검 모드 조회/수정, 관리자 공지 전송, 미인증/일반 사용자 접근 차단, invalid payload 검증 | `npm test`, `npm --prefix src/backend test -- --runTestsByPath tests/system-maintenance.test.js` | 400/401/403/200 응답 및 ADMIN 기준 점검 모드/공지 broadcast 처리 | 통과 |
| TC-INT-019 | 유닛 테스트 | WebSocket helper | WebSocket accept key 생성, 서버 text frame encoding, masked client frame decoding 검증 | `npm test`, `npm --prefix src/backend test -- --runTestsByPath tests/realtime-websocket.test.js` | WebSocket handshake/frame helper가 표준 흐름에 맞게 동작 | 통과 |
| TC-INT-020 | API 통합 테스트 | 친구 간 쪽지 API | 쪽지 thread 목록/생성/상세, 메시지 전송, 읽음 처리, typing recipient 검증, 미인증 접근 차단, 친구 관계 권한, thread 참여자 권한, WebSocket broadcast payload 검증 | `npm test`, `npm --prefix src/backend test -- --runTestsByPath tests/direct-message.test.js` | 400/401/403/200/201 응답 및 accepted 친구 관계 기준 쪽지 처리, 민감정보 미노출 | 통과 |
...
| Backend test | `npm test` | 통과 | Jest + Supertest 전체 백엔드 테스트 통과(29 suites / 501 tests passed) |
...
| Admin Reward API test | `GET/POST/PATCH /api/admin/rewards/badges`, `GET/POST/PATCH /api/admin/rewards/quests` | 통과 | `src/backend/tests/admin-reward.test.js`의 repository mock 기반 API 테스트. 미인증 401, 일반 USER 403, ADMIN 배지/퀘스트 생성·수정·목록 조회, 중복 code 409, 참조 배지 없음 404, invalid quest payload 400 검증 포함 |
| Admin Reward focused test | `npm --prefix src/backend test -- --runTestsByPath tests/admin-reward.test.js` | 통과 | 관리자 보상 배지/퀘스트 CRUD API 단일 테스트 기준 1 suite / 12 tests passed |
...
| Boss Raid API test | `GET /api/boss-raids`, `GET /api/boss-raids/parties/public`, `GET /api/boss-raids/parties/me`, `POST /api/boss-raids/parties`, `POST /api/boss-raids/parties/join`, `POST /api/boss-raids/parties/:partyId/join`, `GET /api/boss-raids/parties/:partyId`, `POST /api/boss-raids/parties/:partyId/claim` | 통과 | `src/backend/tests/boss-raid.test.js`의 repository mock 기반 API 테스트. 미인증 401, 보스 목록, 공개 모집 파티 조회/참여, 비공개 파티 공개 참여 차단, 파티 생성/참가, 파티 상세, 파티 멤버 대상 WebSocket 진행률/완료 event payload, 보상 1회 수령 검증 포함 |
| Boss Raid focused test | `npm --prefix src/backend test -- --runTestsByPath tests/boss-raid.test.js` | 통과 | 스터디 보스 레이드 API 단일 테스트 기준 1 suite / 11 tests passed |
| Collaborative Quest API test | `GET/POST /api/collaborative-quests`, `GET /api/collaborative-quests/:questId`, `POST /api/collaborative-quests/:questId/join`, `POST /api/collaborative-quests/:questId/contributions`, `POST /api/collaborative-quests/:questId/claim` | 통과 | `src/backend/tests/collaborative-quest.test.js`의 repository mock 기반 API 테스트. 미인증 401, 목록/생성/추천 보상 자동 계산, 참여/중복 참여, 만료 퀘스트 참여 차단, 기여도 추가, 완료 event, 미참여자 기여 차단, 보상 1회 수령, 중복 claim 차단 검증 포함 |
| Collaborative Quest focused test | `npm --prefix src/backend test -- --runTestsByPath tests/collaborative-quest.test.js` | 통과 | 협동 퀘스트 API 단일 테스트 기준 1 suite / 14 tests passed |
...
| Direct Message focused test | `npm --prefix src/backend test -- --runTestsByPath tests/direct-message.test.js` | 통과 | 친구 간 쪽지 API 단일 테스트 기준 1 suite / 12 tests passed |
...
| Community Post focused test | `npm --prefix src/backend test -- --runTestsByPath tests/community-post.test.js` | 통과 | 커뮤니티 게시글 API 단일 테스트 파일과 repository query option 및 삭제 안전성 테스트 기준 1 suite / 50 tests passed |
| Community Comment API test | `GET/POST/PATCH/DELETE /api/community/posts/:postId/comments`, `PATCH/DELETE /api/community/comments/:commentId` | 통과 | `src/backend/tests/community-comment.test.js`의 repository mock 기반 API 테스트. 미인증 401, pagination validation, invalid postId/commentId 400, 존재하지 않는 게시글/댓글 404, 대답글 생성/계층 조회/2단 대답글 차단, 댓글/대답글 생성 후 WebSocket broadcast payload, 타인 댓글 수정/삭제 차단, 민감정보 미노출 검증 포함 |
...
| Community Bookmark List focused test | `npm --prefix src/backend test -- --runTestsByPath tests/community-bookmark-list.test.js` | 통과 | 커뮤니티 내 북마크 목록 API 단일 테스트 기준 1 suite / 14 tests passed |
...
| User/Profile account settings 검증 | 사용자 계정 설정, 회원 탈퇴 및 활동 통계 API 검증 | 통과 | `npm test`(29 suites / 501 tests passed), `npm --prefix src/backend test -- --runTestsByPath tests/user-profile.test.js`(1 suite / 29 tests passed) 통과 |
| Voice/Accessibility API 검증 | 음성/접근성 모듈 API 검증 | 통과 | `npm test`(29 suites / 501 tests passed), `npm --prefix src/backend test -- --runTestsByPath tests/accessibility.test.js`(1 suite / 11 tests passed) 통과 |
...
| Direct Message API 검증 | 친구 간 쪽지 API 검증 | 통과 | `npm test`(29 suites / 501 tests passed), `npm --prefix src/backend test -- --runTestsByPath tests/direct-message.test.js`(1 suite / 11 tests passed) 통과 |
...
- `src/backend/tests/direct-message.test.js`
...
- `src/backend/tests/community-bookmark.test.js`
- `src/backend/tests/community-bookmark-list.test.js`
- `src/backend/tests/community-report.test.js`
...
관리자 보상 API 테스트는 repository mock 기반으로 실제 `/api/admin/rewards/badges`, `/api/admin/rewards/quests` route, `authMiddleware`, `adminMiddleware`, service validation 흐름을 통과시키며 관리자 보상 배지/퀘스트 목록 조회, 생성, 수정을 확인함. 미인증 요청은 401, 일반 USER 요청은 403으로 차단하고, 배지/퀘스트 code 중복은 409, 존재하지 않는 참조 배지는 404, 잘못된 퀘스트 type은 400으로 처리되는지 검증함. 응답에는 password, token/JWT 등 민감정보가 포함되지 않으며, 삭제 API와 보상 프론트 화면은 후속 범위로 둠.
...
커뮤니티 북마크 API 테스트는 repository mock 기반으로 실제 Express route, `authMiddleware`, service validation 흐름을 통과시키며 `/api/community/posts/:postId/bookmarks` 북마크 생성/취소 기능을 확인함. 지원하지 않는 field와 `userId` spoofing 차단, 중복 북마크 요청 시 중복 row 방지, 현재 사용자 북마크 취소, 다른 사용자 북마크 영향 없음, 민감정보 미노출을 검증함.

커뮤니티 내 북마크 목록 API 테스트는 repository mock 기반으로 실제 Express route, `authMiddleware`, service validation 흐름을 통과시키며 `/api/community/bookmarks` 목록 조회 기능을 확인함. 현재 사용자 북마크만 반환하고, 다른 사용자 북마크는 목록에 포함하지 않으며, `latest`/`oldest` 정렬과 pagination metadata, 게시글별 `commentCount`, `likeCount`, `dislikeCount`, `bookmarkCount`, `myReaction`, `isBookmarked` 응답을 검증함.
...
향후 랭킹/챌린지, 커뮤니티 프론트 화면, 프론트 화면 자동 테스트를 작성할 때 AI 보조 테스트 스크립트 기록을 항목별로 추가함.
...
| 보상 | 보상 현황 조회, 퀘스트 진행도, 달성 퀘스트 보상 수령, invalid questId, 미달성 퀘스트 수령 차단, 관리자 보상 배지/퀘스트 관리 API, 민감정보 미노출 테스트 완료. 보상 화면은 후속 작성 | 진행 중 |
...
- 테스트 관련 파일
  ...
  - `src/backend/tests/ai.test.js`
  - `src/backend/tests/admin.test.js`
  - `src/backend/tests/admin-reward.test.js`
  - `src/backend/tests/note.test.js`
  - `src/backend/tests/focus-statistics.test.js`
  - `src/backend/tests/reward.test.js`
  - `src/backend/tests/friend.test.js`
