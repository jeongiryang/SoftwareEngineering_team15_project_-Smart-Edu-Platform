...
| Backend test | `npm test` | 통과 | Jest + Supertest 전체 백엔드 테스트 통과(28 suites / 477 tests passed) |
...
| User/Profile account settings 검증 | 사용자 계정 설정, 회원 탈퇴 및 활동 통계 API 검증 | 통과 | `npm test`(28 suites / 477 tests passed), `npm --prefix src/backend test -- --runTestsByPath tests/user-profile.test.js`(1 suite / 26 tests passed) 통과 |
