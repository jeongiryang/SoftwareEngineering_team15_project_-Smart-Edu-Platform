...
현재 백엔드 테스트는 health check, 인증, 사용자/프로필, 학습 일정/태스크, 학습 노트, 커뮤니티 게시글, 관리자, AI 학습 지원 API와 공통 helper 검증을 포함함. 집중 시간/통계는 구현 시 테스트를 추가하고, 커뮤니티는 1차 게시글 CRUD API 테스트를 완료한 뒤 댓글/반응/신고/관리자 연동 테스트를 후속으로 확장함.

### 3.2 통합 테스트 전략

통합 테스트는 Express API 요청/응답, 인증 흐름, DB 연동, Prisma repository 흐름을 중심으로 작성할 예정임.

현재는 `GET /api/health`뿐 아니라 Auth, User/Profile, Schedule/Task, Study Note, Community Post, Admin, AI API를 Jest + Supertest와 repository/provider mock 기반으로 확인함. 커뮤니티는 게시글 목록/상세/작성/수정/삭제와 작성자 권한 검증을 1차로 확인했으며, 댓글/반응/신고/관리자 연동 테스트는 후속 구현 범위에서 추가함.
