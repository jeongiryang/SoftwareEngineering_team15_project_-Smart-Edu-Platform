...
| 소규모 UX | #381 | 회원가입 placeholder/validation, 비밀번호 변경 문구, 점검 화면 문구, 설정 아이콘 | 7.1, 7.14 |
...
이번 프로젝트에서 가장 어려웠던 부분은 코드 자체보다 조장으로서 전체 협업 흐름을 관리하는 일이었다. 브랜치 전략, PR 검증, 팀원 개발 환경 구축, 테스트와 문서 기준을 함께 관리해야 했고, 기능 하나를 구현하는 것보다 변경 범위와 검증 기준을 계속 맞추는 과정이 더 많은 판단을 요구했다. 백엔드 API나 기본 로직보다 UI/UX를 원하는 형태로 구현하는 과정도 어려웠다. 특히 인트로와 서비스 소개 화면에서는 입자 연출, 텍스트 형성, 첫 진입 flicker, 서비스 카드 애니메이션처럼 작은 차이가 전체 완성도에 크게 영향을 주었고, 여러 번 시행착오를 거치며 화면을 조정했다.

AI 에이전트는 성능이 좋은 만큼 원하는 결과를 얻으려면 요구사항을 작게 쪼개고, 제약 조건과 검증 기준을 구체적으로 제시해야 한다는 점을 배웠다. 단순히 코드를 맡기는 것이 아니라 현재 main 상태, 수정 금지 범위, 테스트 명령, 실패 기준을 명확히 주어야 결과를 신뢰할 수 있었다. 이번 프로젝트를 통해 AI 보조 개발 방식에 익숙해졌고, PR, 테스트, 문서, 배포 상태까지 함께 관리하는 경험을 했다.

사각사각은 단순한 기말 대체 과제가 아니라 실제 출시를 목표로 하는 서비스처럼 접근했기 때문에 더 몰입할 수 있었다. 요구사항, 설계, 구현, 테스트, 배포, 문서화가 하나로 이어지면서 소프트웨어공학 이론이 실제 작업 흐름으로 와닿았다. 사용자 관점에서 무엇이 불편할지 고민하며 작은 UI 디테일까지 집요하게 다뤘지만, 일부 부분에서는 욕심이 과해 조원들에게 부담을 주었을 수 있다는 점도 느꼈다. 끝까지 맞춰 주고 피드백해 준 팀원들에게 고맙고, 이번 경험을 통해 기능 구현뿐 아니라 협업 기준을 세우는 것과 팀원 부담을 함께 고려하는 태도가 중요하다는 것을 배웠다.

### 15.2 황대겸

황대겸 팀원은 구현 기능 담당 영역과 QA, 팀 작업 검토에 참여했다. 최종 제출본에는 본인이 맡은 역할, 구현 중 어려웠던 점, 협업 과정에서 배운 점, 다음 프로젝트에서 개선하고 싶은 점을 본인 문장으로 입력해야 한다.

### 15.3 박지환

박지환 팀원은 구현 기능 담당 영역과 QA, 팀 작업 검토에 참여했다. 최종 제출본에는 본인이 맡은 역할, 구현 중 어려웠던 점, 팀원과의 협업 경험, AI 보조 작업을 사용하며 느낀 장단점을 본인 문장으로 입력해야 한다.
