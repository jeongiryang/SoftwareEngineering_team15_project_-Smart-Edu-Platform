# 사각사각 Smart Edu Platform 최종 프로젝트 보고서 초안
...
> 이 문서는 3단계 최종 제출용 보고서 초안이다. 요구사항, 설계, 구현, 테스트, 배포, AI 활용, 협업 운영, 한계와 확장 방향을 하나의 흐름으로 통합해 정리한다. 원문 요구사항과 설계 문서는 부록으로 연결하고, 본문은 제출용 요약과 해석 중심으로 작성한다.
...
학습 노트 API는 노트 생성, 목록 조회, 상세 조회, 수정, 삭제를 지원한다. 노트는 AI 추천, 요약, 오답 분석과 연결될 수 있는 학습 기록의 기반이다. 현재 학습 노트 CRUD는 구현되어 있으며, AI 기반 퀴즈 생성 기능은 요구사항과 모델 초안에 포함되어 있으나 최종 구현에서는 고도화 범위로 남아 있다.

이 보고서에서는 노트 및 퀴즈 생성/관리를 교수님 제시 주제 1의 핵심 요구사항으로 정리하되, 실제 구현 상태는 학습 노트와 AI 보조 흐름 중심으로 설명한다. AI 기반 퀴즈 자동 생성은 구현 완료 기능처럼 과장하지 않고 후속 확장 범위로 표시한다.
...
AI는 요구사항, 설계, 테스트 보고서, API 명세, 설치 가이드, 최종보고서 초안의 구조를 정리하는 데 사용되었다. 최종 제출 문서에서는 raw chat log나 작업 지시문을 그대로 남기지 않고, 결정 사항과 결과 중심으로 요약했다. AI 사용 정책은 [AI 에이전트 사용 및 원격 작업 안전 규칙](../ai-agent-usage-policy.md)에 별도 정리되어 있다.
...
실제 팀원별 최종 소감은 최종 제출 직전 각 팀원이 직접 작성해야 한다. 현재 문서는 작성 위치와 가이드를 제공한다.

| 팀원 | 역할 요약 | 최종 소감 작성 상태 |
|---|---|---|
| 정이량 | 요구사항/설계/문서 구조, PR 검증, frontend/backend 통합, 최종 정리 관리 | 작성 예정 |
| 황대겸 | 구현 기능 담당 영역, QA와 팀 작업 검토 참여 | 작성 예정 |
| 박지환 | 구현 기능 담당 영역, QA와 팀 작업 검토 참여 | 작성 예정 |

소감 작성 시에는 다음 항목을 포함한다.

- 맡은 역할과 기여 내용
- 어려웠던 점
- 해결 과정에서 배운 점
- 팀원 도움과 협업 경험
- AI 보조 작업을 사용하며 느낀 장단점
- 다음 프로젝트에서 개선하고 싶은 점
